...
# img_element = driver.find_element(By.XPATH, RATE_IMG)
# tts_element = driver.find_element(By.XPATH, USD_TTS_PATH)
# ttb_element = driver.find_element(By.XPATH, USD_TTB_PATH)
tts = float(tts_element.text)
ttb = float(ttb_element.text)
spread = tts - ttb
print(f'tts:{tts_element.text}')
print(f'ttb:{ttb_element.text}')
print(f'スプレッド:{spread}')
print(update_datetime.text)
